...
def detect_position_bias(
    ...
    if not bias_detected:
        recommendation = (
            "No position-based action recommended. Confirm with controlled "
            "chunk permutations before changing retrieval order."
        )
        summary = (
        ...
        recommendation = (
            "Run a controlled beginning/middle/end permutation test with the "
            "same evidence. Reorder chunks only if the effect replicates."
        )
        summary = (
    ...
    # --- Step 8: Compute bias intensity score (NEW — continuous 0-1 metric) ---
    if total_chunks >= 3 and middle:
        zone_gap_raw = max(0.0, u_shape_gap)
        zone_gap_norm = min(1.0, zone_gap_raw / 0.25)
        phantom_norm = middle_phantom_ratio
        wasted_norm = min(1.0, len(wasted_good_chunks) / 3.0)
        bias_intensity = round(
            0.45 * zone_gap_norm + 0.35 * phantom_norm + 0.20 * wasted_norm, 3
        )
    else:
        bias_intensity = 0.0

    if bias_intensity >= 0.6:
        intensity_label = "STRONG"
    elif bias_intensity >= 0.3:
        intensity_label = "MODERATE"
    elif bias_intensity > 0.0:
    ...
    return {
        ...
        "bias_intensity_score": bias_intensity,
        "bias_intensity_label": intensity_label,
