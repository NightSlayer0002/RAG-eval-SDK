...
MODEL_NAME = os.getenv("RAG_EVAL_JUDGE_MODEL", "gemini-2.5-flash")
...
def _extract_json(raw_text: str) -> dict:
    ...
    markdown fences, trailing commas, or other wrapper text.

    HARDENED (v2): 4-stage fallback pipeline:
      Stage 1: Direct JSON parse (handles clean output)
      Stage 2: Clean trailing commas + re-parse (handles Gemini's common quirk)
      Stage 3: Regex-extract first {...} block + clean + parse
      Stage 4: Regex-based verdict extraction (last resort — builds JSON from
               raw text by counting SUPPORTED/NOT_SUPPORTED/PARTIAL patterns)

    This reduces parse failures from ~10% to ~2%.
    """
    text = raw_text.strip()
    ...
    # Gemini sometimes outputs: {"key": "value",} or [item1, item2,]
    ...
    # --- Stage 3: Regex extract JSON block + clean ---
    match = re.search(r'\{[\s\S]*\}', text)
    if match:
        block = match.group()
        block_cleaned = re.sub(r',\s*([}\]])', r'\1', block)
        try:
            return json.loads(block_cleaned)
        except json.JSONDecodeError:
            pass

    # --- Stage 4: Regex-based verdict extraction (last resort) ---
    # Even if JSON is completely broken, try to extract useful info
    # by pattern-matching verdict strings from the raw text.
    supported = len(re.findall(r'"verdict"\s*:\s*"SUPPORTED"', text))
    not_supported = len(re.findall(r'"verdict"\s*:\s*"NOT_SUPPORTED"', text))
    partial = len(re.findall(r'"verdict"\s*:\s*"PARTIAL"', text))
    total = supported + not_supported + partial

    if total > 0:
        # Build a synthetic result from regex-extracted verdicts
        faithfulness = round((supported + 0.5 * partial) / total, 3)
        if faithfulness >= 0.8:
        ...
        return {
            "sentences": [],  # can't reliably extract individual sentences
            "overall_faithfulness": faithfulness,
            "overall_verdict": verdict,
            "reasoning": (
                f"Extracted from malformed JSON via regex: "
                f"{supported} SUPPORTED, {not_supported} NOT_SUPPORTED, "
                f"{partial} PARTIAL out of {total} sentences."
            ),
            "_parse_method": "regex_fallback"
        }

    raise json.JSONDecodeError("No valid JSON or verdict patterns found in response", text, 0)
...
def judge_faithfulness(response: str, context_chunks: list) -> dict:
    ...
    try:
        ...
        return {
            ...
            "reasoning": "Optional Google provider dependency is not installed.",
            "latency_seconds": 0,
        }
    if not client:
        return {
